...
PanelWindow {
    ...
    // Manage visibility locally
    visible: false

    // Native Wayland positioning
    anchors {
        top: true
        bottom: false
        left: true
    ...
    IpcHandler {
        target: "monitor_widget"
        function toggle(): void {
            window.visible = !window.visible
            if (window.visible) themeProcess.running = true
        }
    }

    // --- Responsive Scaling Logic (Inline Fallback) ---
    property real baseScale: 1.0
    Component.onCompleted: {
        // Safe relative fallback without requiring external Scaler.qml
        window.baseScale = 1920.0 / 1920.0
        startupAnim.start()
    }

    function s(val) {
        return val * window.baseScale;
    }
                ...
                Item {
                    ...
                    Item {
                        ...
                        property real targetScale: {
                            ...
                            return Math.min(1.8 * window.baseScale, Math.min(window.s(340) / requiredW, window.s(240) / requiredH));
                        }
