...
PanelWindow {
    id: rootWin
    visible: GlobalStates.showNetworkPopup

    // Native Wayland positioning (bypasses Hyprland entirely)
    anchors {
        top: true
        right: true
    }
    ...
    IpcHandler {
        ...
        function toggle(): void {
            GlobalStates.showNetworkPopup = !GlobalStates.showNetworkPopup
        }
    }
    ...
    Item {
        ...
        readonly property string scriptsDir: "/usr/share/quickshell/dms/network"
        ...
        onCurrentConnChanged: {
            ...
            if (currentConn) window.updateInfoNodes()
        }
        ...
        onWifiConnectedChanged: {
            ...
            window.syncCores()
            if (window.currentConn && window.activeMode === "wifi") window.updateInfoNodes()
        }
        ...
        onBtConnectedChanged: {
            window.syncCores()
            if (window.currentConn && window.activeMode === "bt") window.updateInfoNodes()
        }

        readonly property bool currentPower: activeMode === "wifi" ? window.wifiPower === "on" : window.btPower === "on"
        onCurrentPowerChanged: { window.syncCores() }
        ...
        Item {
            anchors.fill: parent
            scale: 0.8 + (0.2 * window.introState)
                    ...
                    Repeater {
                        ...
                        delegate: Item {
                            ...
                            Rectangle {
                                ...
                                property bool isDangerState: coreMa.containsMouse || disconnectFill > 0 || coreContainer.isMyDisconnecting
                                ...
                                gradient: Gradient {
                                    ...
                                    GradientStop {
                                        ...
                                        color: {
                                            ...
                                            if (coreContainer.isMyDisconnecting) return window.surface0
                                    ...
                                    GradientStop {
                                        ...
                                        color: {
                                            ...
                                            if (coreContainer.isMyDisconnecting) return window.base
                                ...
                                Rectangle {
                                    ...
                                    opacity: window.currentConn && !coreContainer.isMyDisconnecting ? (centralCore.isDangerState ? 0.3 : 0.15) : 0.0
                                    z: -1
                                ...
                                Rectangle {
                                    ...
                                    opacity: (window.currentConn && window.showInfoView && window.currentPower && !coreContainer.isMyDisconnecting) ? pulseOp : 0.0
                                    scale: pulseSc
                                ...
                                Item {
                                    ...
                                    ColumnLayout {
                                        ...
                                        Text {
                                            ...
                                            font.family: "Iosevka Nerd Font"
                                            font.pixelSize: 48 - (16 * coreContainer.multiShift)
                                            color: coreContainer.isMyDisconnecting ? window.overlay1 : window.crust
                                            text: coreContainer.isMyDisconnecting ? "" : (coreMa.containsMouse ? (window.activeMode === "wifi" ? "󰖪" : "󰂲") : (coreContainer.myDevice ? coreContainer.myDevice.icon : ""))
                                            Behavior on color { ColorAnimation { duration: 200 } }
                                        }

                                        LoadingDots {
                                            Layout.alignment: Qt.AlignHCenter
                                            visible: coreContainer.isMyDisconnecting
                                        ...
                                        Text {
                                            ...
                                            font.weight: Font.Bold
                                            font.pixelSize: 11
                                            color: coreContainer.isMyDisconnecting ? window.overlay1 : (centralCore.disconnectFill > 0.1 ? window.crust : (coreMa.containsMouse ? window.crust : "#99000000"))
                                            text: coreContainer.isMyDisconnecting ? "Disconnecting..." : (centralCore.disconnectFill > 0.1 ? "Hold..." : "Connected")
                                            Behavior on color { ColorAnimation { duration: 200 } }
                                        }
                                    }
                                    ...
                                    MouseArea {
                                        ...
                                        anchors.fill: parent
                                        hoverEnabled: true
                                        cursorShape: window.currentConn && !coreContainer.isMyDisconnecting ? Qt.PointingHandCursor : Qt.ArrowCursor

                                        onPressed: {
                                            if (window.currentConn && !coreContainer.isMyDisconnecting && !centralCore.disconnectTriggered) {
                                                coreDrainAnim.stop()
                                                coreFillAnim.start()
                                            }
                                        }
                                        onReleased: {
                                            if (!centralCore.disconnectTriggered && !coreContainer.isMyDisconnecting) {
                                                coreFillAnim.stop()
                                                coreDrainAnim.start()
                                            }
                                        }
                                    ...
                                    NumberAnimation {
                                        ...
                                        onFinished: {
                                            ...
                                            let macStr = coreContainer.myDevice ? (coreContainer.myDevice.mac || "") : ""
                                            let cmd = window.activeMode === "wifi"
                                                ? "nmcli device disconnect $(nmcli -t -f DEVICE,TYPE d | grep wifi | cut -d: -f1 | head -n1)"
                                                : "bash " + window.scriptsDir + "/bluetooth_panel_logic.sh --disconnect '" + macStr + "'"
                                            Quickshell.execDetached(["sh", "-c", cmd])
                    ...
                    Item {
                        ...
                        Repeater {
                            ...
                            delegate: Item {
                                ...
                                property int siblingsCount: {
                                    ...
                                    if (m && m.count !== undefined) {
                                        for (let i = 0; i < m.count; i++) {
                                            let d = m.get(i)
                                            if (d && (d.parentIndex !== undefined ? d.parentIndex : -1) === floatCardDelegateContainer.myParentIdx) c++
                                        }
                                    }
                                    return Math.max(1, c)
                                }
                                property int localIndex: {
                                    ...
                                    if (m && m.count !== undefined) {
                                        for (let i = 0; i < index; i++) {
                                            let d = m.get(i)
                                            if (d && (d.parentIndex !== undefined ? d.parentIndex : -1) === floatCardDelegateContainer.myParentIdx) idx++
                                        }
                                    }
                                    return idx
                                }
                                ...
                                property real activeCount: (floatCardDelegateContainer.unifiedRatio > 0.5 && floatCardDelegateContainer.myParentIdx !== -1) ? floatCardDelegateContainer.siblingsCount : orbitRepeater.count
                                property real dynamicScale: floatCardDelegateContainer.activeCount > 10 ? Math.max(0.6, 12.0 / floatCardDelegateContainer.activeCount) : (floatCardDelegateContainer.unifiedRatio > 0.5 ? (window.activeCoreCount > 2 ? 0.7 : 0.8) : 1.0)

                                property real safeMultiShift: window.activeMode === "wifi" ? 0.0 : window.multiTransitionState
                                property var pItem: floatCardDelegateContainer.myParentIdx !== -1 ? coreRepeater.itemAt(floatCardDelegateContainer.myParentIdx) : null

                                property real parentX: floatCardDelegateContainer.pItem ? (orbitContainer.width / 2) + (Math.cos(floatCardDelegateContainer.parentCoreAngle) * floatCardDelegateContainer.pItem.myOrbitRadiusX * floatCardDelegateContainer.safeMultiShift * floatCardDelegateContainer.pItem.activeTransition) : (orbitContainer.width / 2)
                                property real parentY: floatCardDelegateContainer.pItem ? (orbitContainer.height / 2) + (Math.sin(floatCardDelegateContainer.parentCoreAngle) * floatCardDelegateContainer.pItem.myOrbitRadiusY * floatCardDelegateContainer.safeMultiShift * floatCardDelegateContainer.pItem.activeTransition) : (orbitContainer.height / 2)

                                property real parentBaseAngle: floatCardDelegateContainer.pItem ? floatCardDelegateContainer.pItem.animatedBaseAngle : 0
                                ...
                                property real singleLiveAngle: (window.globalOrbitAngle * 1.5) + floatCardDelegateContainer.singleBaseAngle

                                property real arcSpread: Math.PI * 0.8
                                property real nodeOffset: (floatCardDelegateContainer.siblingsCount > 1) ? ((floatCardDelegateContainer.localIndex / (floatCardDelegateContainer.siblingsCount - 1)) - 0.5) * floatCardDelegateContainer.arcSpread : 0
                                property real parentCoreAngle: (window.globalOrbitAngle * 1.5) + floatCardDelegateContainer.parentBaseAngle
                                property real multiLiveAngle: floatCardDelegateContainer.myParentIdx === -1 ? floatCardDelegateContainer.singleLiveAngle : (floatCardDelegateContainer.parentCoreAngle + floatCardDelegateContainer.nodeOffset)

                                property bool isInfoNodeRef: model.isInfoNode !== undefined ? model.isInfoNode : false
                                property int ringIndex: floatCardDelegateContainer.isInfoNodeRef ? 0 : index % 2
                                property real ringOffset: floatCardDelegateContainer.ringIndex * 40

                                property real singleRadX: floatCardDelegateContainer.isInfoNodeRef ? 280 : 320 + floatCardDelegateContainer.ringOffset
                                property real singleRadY: floatCardDelegateContainer.isInfoNodeRef ? 180 : 200 + floatCardDelegateContainer.ringOffset

                                // Scan node (-1) perfectly snaps to dead center (0,0) so it avoids crossing paths with Cores
                                property real multiRadX: floatCardDelegateContainer.isInfoNodeRef ? (floatCardDelegateContainer.myParentIdx === -1 ? 0 : (window.activeCoreCount > 2 ? 180 : 160)) : 340 + floatCardDelegateContainer.ringOffset
                                property real multiRadY: floatCardDelegateContainer.isInfoNodeRef ? (floatCardDelegateContainer.myParentIdx === -1 ? 0 : (window.activeCoreCount > 2 ? 180 : 160)) : 240 + floatCardDelegateContainer.ringOffset

                                property real currentRadX: (floatCardDelegateContainer.singleRadX * (1 - floatCardDelegateContainer.unifiedRatio)) + (floatCardDelegateContainer.multiRadX * floatCardDelegateContainer.unifiedRatio)
                                property real currentRadY: (floatCardDelegateContainer.singleRadY * (1 - floatCardDelegateContainer.unifiedRatio)) + (floatCardDelegateContainer.multiRadY * floatCardDelegateContainer.unifiedRatio)
                                property real currentAngle: (floatCardDelegateContainer.singleLiveAngle * (1 - floatCardDelegateContainer.unifiedRatio)) + (floatCardDelegateContainer.multiLiveAngle * floatCardDelegateContainer.unifiedRatio)
                                ...
                                property real animRadX: (floatCardDelegateContainer.currentRadX + floatCardDelegateContainer.pwrDrift) * floatCardDelegateContainer.entryAnim
                                property real animRadY: (floatCardDelegateContainer.currentRadY + floatCardDelegateContainer.pwrDrift) * floatCardDelegateContainer.entryAnim

                                property real targetX: floatCardDelegateContainer.myParentIdx === -1
                                    ? (orbitContainer.width / 2) - (width / 2) + Math.cos(floatCardDelegateContainer.currentAngle) * floatCardDelegateContainer.animRadX
                                    : floatCardDelegateContainer.parentX - (width / 2) + Math.cos(floatCardDelegateContainer.currentAngle) * floatCardDelegateContainer.animRadX

                                property real targetY: floatCardDelegateContainer.myParentIdx === -1
                                    ? (orbitContainer.height / 2) - (height / 2) + Math.sin(floatCardDelegateContainer.currentAngle) * floatCardDelegateContainer.animRadY
                                    : floatCardDelegateContainer.parentY - (height / 2) + Math.sin(floatCardDelegateContainer.currentAngle) * floatCardDelegateContainer.animRadY

                                property real liveBob: floatCardDelegateContainer.myParentIdx === -1 && floatCardDelegateContainer.isInfoNodeRef
                                    ? Math.sin(window.globalOrbitAngle * 6) * 12 * (1 - floatCardDelegateContainer.unifiedRatio)
                                    : 0

                                x: floatCardDelegateContainer.targetX
                                y: floatCardDelegateContainer.targetY + floatCardDelegateContainer.liveBob

                                scale: (!floatCardDelegateContainer.isLoaded ? 0.0 : (floatMa.pressed ? floatCardDelegateContainer.dynamicScale * 0.95 : (floatCard.locksList ? floatCardDelegateContainer.dynamicScale * 1.08 : floatCardDelegateContainer.dynamicScale))) * floatCard.bumpScale
                                ...
                                Rectangle {
                                    ...
                                    property bool isMyBusy: !!window.busyTasks[floatCard.itemId]

                                    property bool isPairedBT: window.activeMode === "bt" && model.action === "Connect"
                                    property bool isTargetWifi: window.activeMode === "wifi" && !window.isWifiConn && floatCard.itemId === window.targetWifiSsid
                                    property bool isSpecialAction: floatCard.itemId === "action_scan" || floatCard.itemId === "action_settings"
                                    property bool isHighlighted: floatCard.isPairedBT || floatCard.isTargetWifi || floatCard.isSpecialAction

                                    property bool isCurrentlyConnected: {
                                        if (window.activeMode === "wifi") return (window.wifiConnected && window.wifiConnected.ssid === floatCard.itemId)
                                        for (let i = 0; i < window.btConnected.length; i++) {
                                            if (window.btConnected[i].mac === floatCard.itemId) return true
                                        }
                                        return false
                                    }

                                    property bool isActionableRef: model.isActionable !== undefined ? model.isActionable : false
                                    property bool isInteractable: !floatCardDelegateContainer.isInfoNodeRef || floatCard.isActionableRef
                                    property bool locksList: floatCard.isInteractable && (floatMa.containsMouse || floatMa.pressed)
                                    ...
                                    property bool doMarquee: floatMa.containsMouse && floatCard.nameImplicitWidth > floatCard.nameContainerWidth
                                    ...
                                    onDoMarqueeChanged: if (!floatCard.doMarquee) floatCard.textOffset = 0;

                                    property real fillLevel: 0.0
                                    property bool triggered: false
                                    property real flashOpacity: 0.0

                                    property real renderFill: (floatCard.isCurrentlyConnected) ? 1.0 : floatCard.fillLevel

                                    onIsMyBusyChanged: {
                                        if (!floatCard.isMyBusy && floatCard.triggered) {
                                    ...
                                    onIsCurrentlyConnectedChanged: {
                                        if (!floatCard.isCurrentlyConnected && floatCard.fillLevel > 0) drainAnim.start()
                                    }
                                    ...
                                    Rectangle {
                                        ...
                                        border.width: 2
                                        visible: floatCard.isHighlighted && !floatCard.isMyBusy && !floatCard.isCurrentlyConnected
                                    ...
                                    RowLayout {
                                        ...
                                        ColumnLayout {
                                            ...
                                            Text {
                                                ...
                                                text: floatCard.isMyBusy ? "Connecting..." : (floatCard.renderFill > 0.1 && floatCard.renderFill < 1.0 ? "Hold..." : (model.action || ""))
                                                Behavior on color { ColorAnimation { duration: 200 } }
                                            }
                                        }
                                    }
                                    ...
                                    NumberAnimation {
                                        ...
                                        onFinished: {
                                            ...
                                            let cmdStrSafe = model.cmdStr || ""
                                            if (cmdStrSafe === "TOGGLE_VIEW") {
                                                window.playSfx("switch.wav")
                                                window.showInfoView = !window.showInfoView
                                                floatCard.triggered = false
                                                drainAnim.start()
                                            } else if (floatCardDelegateContainer.isInfoNodeRef && cmdStrSafe !== "") {
                                                Quickshell.execDetached(["sh", "-c", cmdStrSafe])
                                                ...
                                                let ssidSafe = model.ssid || ""
                                                let macSafe = model.mac || ""

                                                let cmd = window.activeMode === "wifi"
                                                    ? "nmcli device wifi connect '" + ssidSafe + "'"
                                                    : "bash " + window.scriptsDir + "/bluetooth_panel_logic.sh --connect " + macSafe
